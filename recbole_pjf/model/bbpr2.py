...
class BBPR2(GeneralRecommender):
    input_type = InputType.PAIRWISE

    def __init__(self, config, dataset):
        super(BBPR2, self).__init__(config, dataset)
    ...
    def calculate_loss(self, interaction):
        ...
        score_pos_1 = self.forward(pos_user, pos_item, 1)
        score_neg_i_1 = self.forward(pos_user, neg_item, 1)

        score_pos_2 = self.forward(pos_user, pos_item, 2)
        score_neg_i_2 = self.forward(neg_user, pos_item, 2)

        loss = self.loss(score_pos_1, score_neg_i_1) + self.loss(score_pos_2, score_neg_i_2)
        return loss
