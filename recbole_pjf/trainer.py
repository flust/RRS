...
class MultiDirectTrainer(Trainer):
    ...
    def evaluate(self, eval_data, load_best_model=True, model_file=None, show_progress=False):
        ...
        # calculate Rrecall
        rrecall = unique_rec_num / struct_g.get('rec.topk').sum(dim=0)[-1]
        # calculate Rprecision
        total_rec_count = torch.Tensor([(struct_g.get('rec.topk').shape[0] * (struct_g.get('rec.topk').shape[1] - 1) + 
                                        struct_j.get('rec.topk').shape[0] * (struct_j.get('rec.topk').shape[1] - 1))])
        rprecision = unique_rec_num / total_rec_count
        # calculate Rndcg


        result_dict = dict()
        result_dict['Rrecall'] = rrecall
        result_dict['Rprecision'] = rprecision

        test_result_g.update(result_dict)
        test_result_j.update(result_dict)
    ...
    def _full_sort_batch_eval(self, batched_data):
        ...
        if batch_size <= self.test_batch_size:
            if self.config['model'] in ['BBPR', 'BBPR2', 'BBPRMLP', 'CausCF']:
                scores = self.model.predict(new_inter, self.full_eval_direct)
            else:
